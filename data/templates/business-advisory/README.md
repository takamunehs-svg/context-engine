...
display_name: AIネイティブ事業支援テンプレート
version: 0.1.0
target_industry: AI導入支援・DX推進・経営変革・組織変革コンサルティング
target_tenant: AIアドバイザー / DXコンサルタント / 事業変革支援者
---

# AIネイティブ事業支援テンプレート

context-engine の業界テンプレ第1号。AIアドバイザーやDXコンサルタントが「フレームを使う N社」となり、その下に複数の支援先企業（subject = A社・B社・C社）を抱える運用を想定。
...
このテンプレ内で使われる分類記号（リスク階層 high/medium/watch/low 等）は **このテンプレに閉じる**。メタプラットフォーム（context-engine 本体）には持ち込まない。
