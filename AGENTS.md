...
3層構造（辞書層 / SoA / SoM）+ Episodic Memory（append-only）を、**任意の業界・業務に当てはめて運用できるメタプラットフォーム**。
...
- **辞書層・SoA・SoM・Memory のスキーマはテナント内で1つ**（混ざらない）
...
| SoA | 現場活動の記録・帳簿 | append-only | `soa_events` |
| SoM | 判定・計画・ルール | 上書き可・版管理 | `som_decisions` |
| Memory | 横断的な学習・判断履歴 | append-only | `episodic_memory_entries` |

混ぜたり、SoAをUPDATEしたりしてはならない。

## R-2: Append-only は非交渉

- `soa_events`：UPDATE/DELETE禁止。補正は新イベントで上書き
...
- `industry_templates` テーブル：辞書層・SoAスキーマ・SoMルールをパッケージ化
...
- `src/lib/rules/*.ts`（追加予定）は SoM ルールを `if/else` とテーブルでコード化したもの
- LLM 呼び出しは禁止
...
- `soa_events` / `episodic_memory_entries` への UPDATE/DELETE を含むコード
- マイグレーションの編集・追加
...
- `soa_events` / `episodic_memory_entries` への UPDATE/DELETE 自動実行（R-2 / S-5）
