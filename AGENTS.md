...
## R-10: 意思決定の理由を `PLAN.md §8` に追記

- 非自明な判断をコードに反映したら `D-XXX` として追記する
- **冒頭に「動かす葉: L1〜L14」を必須記載**（価値ツリーは PLAN.md §7 参照、Platform Revolution 由来の L11-L14 を含む）
...
- **Curation 機構を伴わない第三者コンテンツ受け入れ機能**（Phase 0-1 では Producer = 渡邊本人のみ。Curation は Phase 2 で稼働、SPEC.md §5.4 参照）
...
- **Curation なき第三者テンプレ受け入れ**（Chatroulette 失敗例。SPEC.md §5.4 / §9）
